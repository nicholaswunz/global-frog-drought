...
micro_raindef <- micro_global_raindef(runmoist=T, nyear=2, raindef=0.2)

soilmoist <- as.data.frame(micro$soilmoist)
soilmoist_raindef <- as.data.frame(micro_raindef$soilmoist)
metout <- as.data.frame(micro$metout)
metout_raindef <- as.data.frame(micro_raindef$metout)

plot(micro$RAINFALL, type='l', ylim=c(0,100), main='rainfall')
points(micro_raindef$RAINFALL, type='l', col='red')

plot(soilmoist$WC2.5cm, type='l', main='water content 2.5 cm in the soil')
points(soilmoist_raindef$WC2.5cm, type='l', col='red')

plot(metout$RHLOC, type='l', ylim=c(0,100), main='RH at animals location')
points(metout_raindef$RHLOC, type='l', col='red')
