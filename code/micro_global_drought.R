...
micro_global_drought <- function(
  ...
  if(errors==0){ # continue
    ...
    # first for min RH
    RHMINN <- RHMINN * rhfact # drop it by the factor you're after
    # then for max RH
    RHMAXX <- RHMAXX * rhfact
...
# library(NicheMapR)
# 
# micro <- micro_global(runmoist=T, nyear=2)
# micro_drought <- micro_global_drought(runmoist=T, nyear=2, 
#                                       rhfact=0.5, rainfact=0.5)
# 
# soilmoist <- as.data.frame(micro$soilmoist)
# soilmoist_drought <- as.data.frame(micro_drought$soilmoist)
# metout <- as.data.frame(micro$metout)
# metout_drought <- as.data.frame(micro_drought$metout)
# 
# plot(metout$RH, type='l', ylim=c(0,100), main='RH')
# points(metout_drought$RH, type='l', col='blue')
# 
# plot(metout$RHLOC, type='l', ylim=c(0,100), main='RH at animals location')
# points(metout_drought$RHLOC, type='l', col='blue')
# 
# plot(micro$RAINFALL, type='l', ylim=c(0,100), main='rainfall')
# points(micro_drought$RAINFALL, type='l', col='red')
# 
# plot(soilmoist$WC2.5cm, type='l', main='water content 2.5 cm in the soil')
# points(soilmoist_drought$WC2.5cm, type='l', col='red')
