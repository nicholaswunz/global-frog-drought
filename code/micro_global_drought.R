...
micro_global_drought <- function(
  ...
  if(errors==0){ # continue
    ...
    # correct for drought factor
    # first for min RH
    VPMINNsat <- WETAIR(db = TMAXX, rh = 100)$esat # compute saturation vap pressure 
    VPMINN <- WETAIR(db = TMAXX, rh = RHMINN)$e # compute vap pressure 
    VPMINNd <- VPMINN * dfact # drop it by the factor you're after
    RHMINN <- (VPMINNd / VPMINNsat) * 100 # convert back to RH
    # then for max RH
    VPMAXXsat <- WETAIR(db = TMINN, rh = 100)$esat
    VPMAXX <- WETAIR(db = TMINN, rh = RHMAXX)$e
    VPMAXXd <- VPMAXX * dfact
    RHMAXX <- (VPMAXXd / VPMAXXsat) * 100
...
# second version of micro_global_drought
# applying the drought factor directly to RH

micro_global_drought2 <- function(
  dfact = 0.8,
  loc = c(-89.4557, 43.1379),
  timeinterval = 12,
  nyears = 1,
  ...
  if(errors==0){ # continue
    ...
    # correct for drought factor
    # first for min RH
    RHMINN <- RHMINN * dfact # drop it by the factor you're after
    # then for max RH
    RHMAXX <- RHMAXX * dfact
...
micro <- micro_global()
micro_drought <- micro_global_drought(dfact=0.8)
micro_drought2 <- micro_global_drought2(dfact=0.8)

metout <- as.data.frame(micro$metout)
metout_drought <- as.data.frame(micro_drought$metout)
metout_drought2 <- as.data.frame(micro_drought2$metout)

plot(metout$RH, type='l', ylim=c(0,100), main='RH')
points(metout_drought$RH, type='l', col='red')
points(metout_drought2$RH, type='l', col='blue')

plot(metout$RHLOC, type='l', ylim=c(0,100), main='RH at animals location')
points(metout_drought$RHLOC, type='l', col='red')
points(metout_drought2$RHLOC, type='l', col='blue')
